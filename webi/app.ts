...
function initRevealOnScroll(): void {
  ...
  targets.forEach((t, i) => {
    ...
    (t as HTMLElement).style.transitionDelay = `${(i % 4) * 60}ms`;
    observer.observe(t);
  });
}
...
function initMeters(): void {
  const meters = Array.from(document.querySelectorAll<HTMLElement>('.meter'));
  const certBars = Array.from(document.querySelectorAll<HTMLElement>('.cert-bar-fill'));

  const fillOne = (el: HTMLElement, targetEl: HTMLElement): void => {
...
function buildTopology(): void {
  const svg = document.getElementById('topoSvg') as unknown as SVGSVGElement | null;
  if (!svg) return;
  ...
  if (!linksGroup || !nodesGroup || !packetsGroup) return;
  ...
  const nodeById = (id: string): TopoNode =>
    nodes.find((n) => n.id === id) as TopoNode;
...
/* ---------------------------------------------------------------------
   ...
   --------------------------------------------------------------------- */
function initPingForm(): void {
  const form = document.getElementById('pingForm') as HTMLFormElement | null;
  const output = document.getElementById('pingOutput') as HTMLElement | null;
  if (!form || !output) return;

  form.addEventListener('submit', (e: SubmitEvent) => {
    e.preventDefault();

    const nameEl = document.getElementById('fname') as HTMLInputElement;
    const emailEl = document.getElementById('femail') as HTMLInputElement;
    const msgEl = document.getElementById('fmsg') as HTMLTextAreaElement;
